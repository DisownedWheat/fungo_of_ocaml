...
type lexer_result = Ok of state | Err of lexer_error | Finished of state
...
let make_token state kind value =
  {
    state with
    tokens =
      Token.make ~line:state.current_line ~column:state.current_column ~value
        ~position:
          (state.current_position - String.length value, state.current_position)
        ~kind
      :: state.tokens;
  }
    ...
    actual_kind
      {
        ...
        position =
          (state.current_position - String.length str, state.current_position);
        value = str;
      }
  in
...
let rec lex_number state =
  ...
  | c :: rest ->
      if not (check_num_char c) then collapse_number state
...
let lex_char state =
  ...
  | '\'' :: char :: '\'' :: rest ->
      Ok
        (make_token state (fun s -> CharLiteral s) (Char.to_string char)
        |> move_state rest)
  | _ ->
      Err
        (InvalidCharLiteral (state.current_position, state.current_position + 1))
...
let rec lex_op state =
  ...
  | [], '&' :: next :: rest when check_ident_char next ->
      Ok
        (move_state rest
           (make_token state (fun s -> Operator (Pointer, s)) "&"))
  | [], '*' :: next :: rest when check_ident_char next ->
...
let rec lex_comment state buffer =
  ...
  | '\n' :: rest ->
      Ok
        (move_state rest
           (make_token state (fun s -> Comment s) (collapse buffer)))
...
let rec inner_lex state_result =
  ...
  | Ok state ->
      ...
      | '[' :: rest ->
          Ok
            (make_token state (fun s -> Operator (LBracket, s)) "["
            |> move_state rest)
      | ']' :: rest ->
          Ok
            (make_token state (fun s -> Operator (RBracket, s)) "]"
            |> move_state rest)
      | '(' :: rest ->
          Ok
            (make_token state (fun s -> Operator (LParen, s)) "("
            |> move_state rest)
      | ')' :: rest ->
          Ok
            (make_token state (fun s -> Operator (RParen, s)) ")"
            |> move_state rest)
      | '{' :: rest ->
          Ok
            (make_token state (fun s -> Operator (LBrace, s)) "{"
            |> move_state rest)
      | '}' :: rest ->
          Ok
            (make_token state (fun s -> Operator (RBrace, s)) "}"
            |> move_state rest)
